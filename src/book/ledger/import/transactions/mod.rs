...
fn import_fiscal_year(ledger: &mut Ledger, banks: &mut BankAccounts, fiscal_year_settings: &settings::FiscalYear) -> BookResult {
...
pub fn import_using_settings(ledger: &mut Ledger, banks: &mut BankAccounts, settings: &settings::Settings) -> BookResult {
    ...
    for fiscal_year in settings.fiscal_years.iter() {
        import_fiscal_year(ledger, banks, fiscal_year)?;
    }
    Ok(())
}