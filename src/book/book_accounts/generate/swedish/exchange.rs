...
impl Associable<Exchange, Params<'_>> for AssociableExchange {
    fn associate(&mut self, _ledger_id: LedgerId, event: &Exchange, p: &mut Params) -> BookResult<AssociableChange> {
        ...
        if self.exchange.date==event.date {
            ...
            let currency_change = BookAccountAmount::from_signed_amount(dir.book_amount);
            let book_change = BookAccountAmount::from_signed_amount(dir.realised_book_amount);
            let correction = BookAccountAmount::from_signed_amount(dir.realised_book_amount+dir.book_amount).invert();
