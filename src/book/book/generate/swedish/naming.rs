...
pub fn generate_naming(accounts: &mut Book) {
    accounts.set_account_name(CLAIMS_TO_CUSTOMERS, "Claims to customers");
    accounts.set_account_name(INCOMING_MOMS, "Incoming moms");
    accounts.set_account_name(OUTGOING_MOMS_REVERSE_CHARGE_25_PERCENT, "Outgoing moms, reverse charge, 25%");
    ...
    accounts.set_account_name(CLAIMS_FROM_SUPPLIERS, "Claims from suppliers");
    ...
    accounts.set_account_name(UNPAID_SHARES, "Unpaid shares");
    accounts.set_account_name(EXCHANGE_RATE_DIFFERENCES, "Exchange rate differences");
}
